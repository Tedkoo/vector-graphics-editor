    ...
    public partial class FormMain : Form, IDrawable
    {
        ...
        public FormMain()
        {
            ...
            public void DrawStar(Star star)
            {
                ...
                using(SolidBrush brush = new SolidBrush(star.InsideColor))
                {
                    using (Pen pen = new Pen(star.BorderColor, 10))
                    {
                    points[0] = new Point(star.Location.X + star.Side / 2, star.Location.Y);                                // Top
                    points[1] = new Point(star.Location.X + (int)(0.62 * star.Side), star.Location.Y + (int)(0.38 * star.Radius));  // Upper right indent
                    points[2] = new Point(star.Location.X + star.Side, star.Location.Y + (int)(0.38 * star.Radius));         // Right tip
                    points[3] = new Point(star.Location.X + (int)(0.69 * star.Side), star.Location.Y + (int)(0.62 * star.Radius));  // Lower right indent
                    points[4] = new Point(star.Location.X + (int)(0.81 * star.Side), star.Location.Y + star.Radius);         // Bottom right tip
                    points[5] = new Point(star.Location.X + star.Side / 2, star.Location.Y + (int)(0.76 * star.Radius));     // Bottom center indent
                    points[6] = new Point(star.Location.X + (int)(0.19 * star.Side), star.Location.Y + star.Radius);         // Bottom left tip
                    points[7] = new Point(star.Location.X + (int)(0.31 * star.Side), star.Location.Y + (int)(0.62 * star.Radius));  // Lower left indent
                    points[8] = new Point(star.Location.X, star.Location.Y + (int)(0.38 * star.Radius));                      // Left tip
                    points[9] = new Point(star.Location.X + (int)(0.38 * star.Side), star.Location.Y + (int)(0.38 * star.Radius));  // Upper left indent
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            // Рисува всички моливи до момента
            foreach (var p in allPencils)
                p.Draw(e.Graphics);

            // Рисува текущата линия, докато рисуваш
            currentPencil?.Draw(e.Graphics);
        ...
        private void FormMain_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (pencilMode && e.Button == MouseButtons.Left)
            {
                currentPencil = new Pencil();
                currentPencil.Start(e.Location);
            }
        }
        private void FormMain_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (pencilMode && e.Button == MouseButtons.Left && currentPencil != null)
            {
                currentPencil.AddPoint(e.Location);
                Invalidate();
            }
        }
        ...
        private void selectRightToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            for (int s = 0; s < _shapes.Count; s++)
            {
                ...
                else if (_shapes[s] is Star star)
                {
                    if (star.Location.X  + star.Radius >= middle)
                        SelectShape(star, true);
                }
            }
            Invalidate();
        }
        ...
        private void btnStar_Click(object sender, EventArgs e) { currentShape = "Star";  }
        ...
        private List<Pencil> allPencils = new List<Pencil>(); // ако искаш да пазиш всички линии
        private void PencilBut_Click(object sender, EventArgs e)
        {
            pencilMode = true;
        }
    }
